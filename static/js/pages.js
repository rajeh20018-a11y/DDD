...
if(interest)interest.onsubmit=async e=>{e.preventDefault();const m=qs('#standaloneMessage'),button=interest.querySelector('button[type="submit"]'),f=new FormData(interest),data=Object.fromEntries(f.entries());data.privacy_consent=f.has('privacy_consent');data.marketing_consent=f.has('marketing_consent');data.path=location.pathname;data.visitor_id=VISITOR_ID;button.disabled=true;m.textContent='جارٍ إرسال بياناتك…';try{const r=await fetch(`${API_BASE}/api/interests`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}),out=await readApiResponse(r);if(!r.ok)throw new Error(out.detail||out.error);interest.reset();m.style.color='#246044';m.innerHTML=`<h2>شكرًا لتسجيل اهتمامك في AsirX</h2><p>${THANK_YOU_MESSAGE.replace(/\n/g, '<br>')}</p><div class="success-actions"><a class="btn" href="/">العودة للرئيسية</a><a class="btn track-whatsapp" href="${buildWhatsAppLink()}" target="_blank" rel="noopener">التواصل عبر واتساب</a><a class="btn" href="/destinations.html">استكشاف الوجهات</a></div>`}catch(error){m.style.color='#922e25';m.textContent=error.message||'تعذر حفظ البيانات. حاول مرة أخرى.'}finally{button.disabled=false}};
function track(event){if(event==='site_visit'&&sessionStorage.getItem('asirx_visit_counted'))return;fetch(`${API_BASE}/api/analytics`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event,path:location.pathname,visitor_id:VISITOR_ID})}).then(()=>{if(event==='site_visit')sessionStorage.setItem('asirx_visit_counted','1')}).catch(()=>{})}track('site_visit');if(interest)track('interest_form_open');document.addEventListener('click',e=>{const a=e.target.closest('a,button');if(!a)return;const href=a.getAttribute('href')||'';if(href.includes('tours-360'))track('tour_360_click');if(href.includes('trip-planner')||a.closest('#fullPlanner'))track('trip_planner_click');if(href.includes('wa.me'))track('whatsapp_click')});
