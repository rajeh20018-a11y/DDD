...
const planner=qs('#fullPlanner');if(planner)planner.onsubmit=async e=>{e.preventDefault();const f=new FormData(planner),r=await fetch(`${API_BASE}/api/plan`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({days:f.get('days'),budget:f.get('budget'),interests:f.getAll('interests')})}),p=await r.json();qs('#fullPlanResult').innerHTML=p.schedule.map(d=>`<article class="result-day"><b>اليوم ${d.day}</b><h3>${d.title}</h3>${d.places.map(x=>`<p>${x.icon} ${x.name}</p>`).join('')}</article>`).join('')+`<div class="notice">تم إنشاء الرحلة، سيتم نقلك الآن لإكمال التسجيل…</div>`;setTimeout(()=>location.href='/interest.html?experience='+encodeURIComponent('تخطيط الرحلات'),1400)};
...
if(interest)interest.onsubmit=async e=>{e.preventDefault();const f=new FormData(interest),data=Object.fromEntries(f.entries());data.privacy_consent=f.has('privacy_consent');data.marketing_consent=f.has('marketing_consent');const r=await fetch(`${API_BASE}/api/interests`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}),out=await r.json(),m=qs('#standaloneMessage');m.textContent=out.message||out.error;if(r.ok){interest.reset();m.style.color='#246044'}else m.style.color='#922e25'};
